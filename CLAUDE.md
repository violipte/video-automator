...
- `GET  /api/temas/resolver-coluna?pipeline_id=X|nome=Y|template_id=Z` - índice da coluna. **Regra dura: NUNCA endereçar coluna por índice fixo** (drag&drop reordena). Ambíguo → `ok:false` + candidatos.
- `POST /api/temas/ensure-linha` - garante a linha da data (`{data:"DD/MM/YYYY"}`), **idempotente e atômico**. Insere em ordem cronológica e **reindexa as chaves `{row}_{col}`** das linhas abaixo (mesma lógica do `_shift_celulas_linha` do coringa). É o único ponto com race real no modelo célula-a-célula → resolvido sob o lock.
- `PUT  /api/temas/celula` - **escreve UMA célula de CONTEÚDO** (`tema/titulo/thumb/roteiro/pipeline_id...`), com guard **dentro do lock** (checar fora e gravar depois abriria TOCTOU). Body: `{row|data, col|pipeline_id|canal, campos{}, sobrescrever?}`. Guards: célula **publicada** (`youtube_video_id`) = imutável sempre · célula **ocupada** exige `sobrescrever:true` · **título ≤100 CODEPOINTS** (`len()` do Python — emoji com variation selector conta +1 escondido). Responde com re-GET de validação. Usado pela skill **arquitetar-canal** (FASE H) — substitui o `POST /api/temas` (grid inteiro, 4.4MB) para escrita programática. Contrato validado com o Claude do Carlos. Testado contra cópia do grid real (1896 células, 0 perdidas/embaralhadas no round-trip de insert de linha).
...
- **Thumb** (1 por vez, chrome_profile único; **TODOS os canais renderizados**, não só o piloto de upload — Piter 31/07): fala os **3 modos do Prompt Calendar** exatamente como lá, chamando o próprio Calendar (:8889):
  - **normal** — substituição mecânica; pool de cenas quando existe, **cena embutida no prompt** quando não (ASH/CO3/CO4 — NÃO exigir pool!);
  - **llm** — `POST /api/llm-generate` (template = system prompt, título+thumb da célula = user; keys em `llm-keys.json` DO LADO DO CALENDAR; v1 usa cache, v2-4 `force=true` → variação via temperatura). ENO/TTM/EST; must-have p/ canais futuros;
  - **compose** — `POST /api/compose-thumb` devolve o **PNG pronto** (bg+fonte+gradiente no Calendar; sem Nano Banana, sem revisor). Template `CO` cobre CO1 **e** CO2 (`TEMPLATE_DE`).
  Templates lidos **live** do Calendar (edição lá vale aqui na hora; backup local só se o serviço cair). ⚠️ Ao editar um system prompt de modo llm: **atualizar também o EXEMPLO no final** — o LLM imita o exemplo mais do que obedece a regra (causa raiz do texto pequeno/vazado do ENO, 31/07); e **reiniciar `prompt-calendar.service`** (cache de prompts por data em memória). ENO: conceito visual = pôr do sol fixo + silhueta, texto gigante (20%/linha, linha 1 branca SÓLIDA, linha 2 amarela) — aprovado Piter 31/07. A melhor thumb é **copiada pra `Automator Exports/{data}/Thumbs/{ALIAS}.jpg|png`** — canal sem upload automático pega ali pro manual; o estágio de upload fecha esses como `thumb_only`. Projeto do Flow **por DATA** (`_esteira/flow_projects.json`); marcador `[CANAL dd-mm vN]` impede casamento cruzado; projeto quebrado → registro apagado. **Popups de anúncio do Flow** ("Community Gallery is Live!" etc.) bloqueiam a UI e derrubavam o driver → `thumb_nano._dispensar_avisos` (Escape + botões por regex multi-idioma) + retry do `_abrir_projeto`.
- **Upload** (paralelo entre canais, serial dentro): 1 thread por canal YouTube (CO3/CO4 = mesma thread). `upload_one --no-pin --publish-utc <slot_map()>`. **Horário: fonte de verdade = CADASTRO DO CANAL no Painel Youtube** (Supabase `automator_aliases`+`publish_slots`+`timezone`, lido via `/api/painel-yt/canais`) — Piter edita o horário na aba e a esteira acompanha em ≤10min (TTL). Cadeia: painel → cache em disco (`_esteira/slot_map_cache.json`) → `SLOT_MAP` hardcoded (fallback; upload nunca fica sem horário). Divergência painel×fallback é logada. Canal novo cadastrado no painel (com aliases+slots) entra no mapa automaticamente — o gate de QUEM sobe segue sendo `upload_trigger.canais` no worker_config. `_LockCanal` **espera a vez** (45min) e, estourando, registra dead-letter — nunca skip silencioso.
- **Pin** (REGRA Piter 31/07: **paralelo, máximo 5 por vez** — `pin_concurrency`=5, validado com teste real de 5 profiles + automação CDP, 0 erros): `upload_verify --ensure-pin` (unlisted → pina pelo **commentID** via AdsPower → reagenda → confirma). 1ª tentativa só em **T+12min do upload** (janela "We're processing this video" — sem seção de comentários; evidência 01/08), retries a cada 15min, 3 tentativas. **AdsPower aguenta vários profiles abertos** (Piter opera assim manualmente) — as blindagens da automação são: throttle da Local API (start/stop ~1 req/s, lock em `lib/adspower.py`), **lock por profile** (CO3/CO4 dividem; daily+esteira juntos) e o cap global (`pinlock`). Profile já aberto (uso manual do Piter) **não é fechado** no final. AdsPower fechado (reboot/crash) **auto-abre**: `garantir_app()` em `lib/adspower.py` detecta conexão recusada na Local API, relança o exe (lock: 1 launcher só entre 5 pins paralelos) e espera a API subir (~11s); o retry existente completa a chamada. Validado 31/07 com o app morto de verdade (profile aberto em 26s, zero intervenção). Exe em `C:\Program Files\AdsPower Global\` (override: env `ADSPOWER_EXE`).
...
- **PIN é SEMPRE pelo commentID** (regra do Piter 31/07). O `youtube_comment_id` persiste na célula (gravado pelo reconciliador quando o CTA é postado/identificado) e o `pin.py` navega `watch?v=X&lc=<id>` — comentário em **highlight**, sempre o certo. NUNCA "o comentário mais recente" (`commentThreads order=time`): em vídeo já público isso fixaria comentário de **espectador**. Célula antiga sem o id: `upload_verify` resolve **1×** por dupla checagem (autor == o próprio canal **e** texto == `build_comment`) e devolve pro grid persistir; sem certeza → **não pina**.
- **Erro transitório de upload re-tenta na esteira** (backoff 2/4/8/15min, 4×) antes do dead-letter — um blip de proxy custava 1 dia. Terminal (MP4/título inválido, journal indeterminado) → `falha` direto. Absorvido do youtube-publish-app (app do amigo do Piter), assim como o **lock de instância única** do esteira_worker (2 instâncias pegariam a mesma tarefa; aconteceu de verdade em 31/07 — watchdog + start manual).
- **Profile do AdsPower vem SEMPRE do Supabase** (`canais_yt.adspower_profile_id`), NUNCA por ordem de lista/palpite. Erro real (01/08): debug do pin abriu o vídeo do ENO no profile do **DE** (id chutado da ordem do teste dos 5 profiles) — vídeo unlisted de um canal visto pelo proxy de OUTRO, violando a regra "canal sempre no SEU proxy". ENO=`k1e1yiml`, DE=`k1e1okn1`.
- **Menu de comentário do YouTube tem markup NOVO** (`yt-list-item-view-model`/`yt-sheet-view-model`, rollout ~08/2026) — o xpath antigo (`role="menuitem"`) dava timeout MUDO no pin. `lib/pin.py` aceita os dois; consent overlay (`ytd-consent-bump-v2-lightbox`) é dispensado e sessão deslogada gera erro claro ("precisa RE-LOGIN") em vez de timeout.
- **Pin do comentário — arquitetura final (01/08, 4 causas-raiz na MESMA noite)**: (1) `filter(has_text=regex ancorado)` testa **textContent** e o Polymer injeta `<style>` DENTRO do `tp-yt-paper-item` → `^Pin` nunca casa com "Pin" na tela; (2) `.click()` **sintético de JS fecha o menu SEM disparar a ação** (Polymer) — clique tem que ser evento real (Playwright `get_by_role("menuitem", name=re)` — nome ACESSÍVEL é limpo — ou `page.mouse.click` em coords do JS); (3) o **diálogo de confirmação** ("Fixar este comentário no topo?"/"Substituir") monta **DIRETO no body** via overlay manager, FORA do `ytd-popup-container` — buscar no documento inteiro, labels `Pin|Fixar|Substituir|Replace`; (4) badge "Fixado" é fail-soft e gerava **falso positivo** (`pinned=True` sem fixar — provado visualmente no ENO 06/08) → **PÓS-PROVA OBRIGATÓRIA**: reabrir o menu e exigir `Unpin/Desafixar`, senão RuntimeError. Anti-flicker: máx 1 re-clique por sessão; não fixou → falha honesta → esteira re-tenta (pool 13-19min) em sessão nova. **Navegação SEM `&lc`**: fixar a partir da vista "Comentário em destaque" NÃO abre o diálogo (ENO 07/08: 6 falhas com &lc, probe sem &lc fixou em 1s) — o alvo é achado pelo **permalink** (`a[href*=commentID]`), que preserva a regra do commentID e mata o fallback "primeiro thread" (fixaria espectador em vídeo público). Diálogo demora até dezenas de s em browser carregado (janela 30s + resgate tardio na pós-prova). Abas SEMPRE fecham (inclusive no erro — 6 abas zumbis degradaram o browser do ENO). **Dialog em shadow root FECHADO** (ENO): querySelectorAll, frames e get_by_role TODOS cegos com o dialog na tela (screenshot provava) — rota final = **CDP `Accessibility.queryAXTree`** (nome acessivel "Fixar no topo") + `DOM.getBoxModel` → `page.mouse.click` na coordenada; e' o branch 4 do confirm e do resgate tardio.
- **ID do YouTube pode COMEÇAR COM HÍFEN** (`-v_rUupUGEo`, EOA 09/08): `--video-id -v...` mata o argparse na PARTIDA ("expected one argument") e a falha vira "pin falhou" mudo — TODA passagem de video_id/comment_id via CLI usa a forma `--opt=valor`. Falha gera **dump forense** (`_pin_debug/`: screenshot + textos do menu) — NUNCA debugar pin sem ler isso primeiro.
- **Vídeo `private` (agendado) BLOQUEIA tudo de comentário** (postar, fixar, e até VER — "Não é possível adicionar comentários a vídeos privados"). Toda operação exige o ciclo `private→unlisted→agir→reagendar`. Corolários: (a) flips demais disparam **re-processamento** do vídeo ("O vídeo está sendo processado") e nessa janela a seção de comentários NEM RENDERIZA (pin impossível — esperar 1-2h); (b) **automação e mão humana NUNCA no mesmo vídeo ao mesmo tempo** — o reagendar automático no finally matou um pin manual do Piter no meio do clique (01/08); congelar o canal antes de inspeção manual.
- **CTA duplicado**: o check antigo "existe comentário?" era `count>0` — comentário de ESPECTADOR contava como CTA, e lag de indexação re-postava (2 cópias com 18s de diferença no ENO 06/08). Regra atual: existência = `comment_id` do grid OU match autor==canal + prefixo do `build_comment` (45 chars normalizados); dedup = manter o cid do grid (ou o mais antigo) e apagar extras via `comments.delete`.
- **Reciclar render workers: `job_atual=None` NÃO significa ocioso** — é o ponto cego do heartbeat (task #52): o VPS solta o job por timeout enquanto o worker ainda renderiza. Matou o render do CO1 05/08 a 99% (31/07). Antes de matar workers, conferir também se há `*.mp4.tmp` **crescendo** em `Automator Exports/*/Videos/`. A autocura (re-queue por stale no poll dos workers) re-enfileira o job perdido em ~5-15min — não re-enfileirar na mão em paralelo.
