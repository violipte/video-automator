...
- **Achar/clicar o item "Pin" do menu é por JS `page.evaluate`, NÃO por locator do Playwright** (01/08). Saga real (EN2 06/08, 6 tentativas falhas "nem Pin nem Unpin"): (1) `filter(has_text=regex ancorado)` testa o **textContent**, e no markup Polymer velho (`tp-yt-paper-item`) o Polymer injeta `<style>` DENTRO do item → texto começa com CSS e `^Pin` nunca casa; (2) até **iterando** com `locator.nth(i).inner_text()` o loop LIA "Pin" (ASCII puro, provado por dump de codepoints) e o match não refletia — inexplicado, alguma dessincronia do sync-API. O `evaluate` roda atômico no renderer (querySelectorAll → filtra visíveis por `getClientRects` → testa `innerText` → `.click()` no elemento) e fixou de primeira. Falha do pin agora gera **dump forense automático** (`_pin_debug/`: screenshot + textos do menu + `ultimo_dump.txt`) embutido na mensagem do erro — NUNCA debugar pin sem ler isso primeiro.
