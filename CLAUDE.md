...
- **Pin** (paralelo por canal, cap `pin_concurrency`=3): `upload_verify --ensure-pin` (unlisted → pina pelo **commentID** via AdsPower → reagenda → confirma). 2 tentativas com 10min. **AdsPower aguenta vários profiles abertos** (Piter 31/07, opera assim manualmente) — as blindagens da automação são: throttle da Local API (start/stop ~1 req/s, lock em `lib/adspower.py`), **lock por profile** (CO3/CO4 dividem; daily+esteira juntos) e o cap global (`pinlock`). Profile já aberto (uso manual do Piter) **não é fechado** no final.
